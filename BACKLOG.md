...
- [x] Portal do Fornecedor — link de leitura tokenizado, sem conta DIS, por linha de custo (`/[locale]/fornecedor/[token]`, gerado no Orçamento). Mostra cronograma partilhado + (só para `catering`) confirmados e alergias; nunca preços nem outros fornecedores. Expira 7 dias após o evento, revogável à mão, com o logo/marca da agência no topo e menção discreta ao DIS no rodapé. Decidido em conversa antes de construir: modelo de acesso (token sem login), quem emite (agência ou casal, quem puder editar essa linha), sem cobertura para fornecedores sem linha de custo no MVP
