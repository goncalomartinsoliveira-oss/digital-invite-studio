...
- [x] Moodboard — mural de inspiração partilhado entre agência e casal, área "Inspiração" no painel do evento. MVP simples: grid estilo masonry, carregar imagem ou colar link (Pinterest/Instagram, com miniatura extraída no servidor via `/api/moodboard/unfurl` — CORS não permite ir buscar isso do browser), legenda opcional por item, sem categorias/álbuns. Sempre partilhado (sem distinção agency/shared, ao contrário de Orçamento/Tarefas — nada aqui é sensível). Decidido em conversa: MVP sem categorias, e ir buscar a miniatura automaticamente (em vez de só mostrar o link em texto), com as cautelas habituais de um endereço fornecido por alguém (só autenticado, timeout curto, bloqueio a endereços de rede interna)
