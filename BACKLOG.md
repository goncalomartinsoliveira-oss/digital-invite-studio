...
- [ ] **Testar com as 2 agências piloto durante um ciclo real (6-8 semanas)**, antes de continuar a construir. A pergunta não é "gostam?", é "deixaram de abrir o Excel?" — já vamos com Fase 0, 1a, 1b e 2 construídas sem esse feedback
- [x] Fase 2 — linhas de custo por pessoa ligadas aos confirmados ao vivo (já existia desde a Fase 0), resumo de alergias para o catering (já existia em `GuestsModule`, painel "DIETARY CARD" + export PDF filtrado), cronograma do dia com exportação PDF (`TimelineModule` — bloco a bloco, partilhado com o casal por omissão, ao contrário do resto do módulo)
- [x] Portal do Fornecedor — link de leitura tokenizado, sem conta DIS (`/[locale]/fornecedor/[token]`). Nova área "Partilha" no painel do evento, com dois tipos de link por evento (não por fornecedor, redesenhado em conversa depois da primeira versão): "Só Cronograma" e "Informação completa" (cronograma + confirmados + alergias) — o mesmo link pode ser enviado a vários fornecedores; nunca inclui orçamento/preços. Expira 7 dias após o evento, revogável/regenerável à mão (afeta todos os que o receberam), com o logo/marca da agência no topo e menção discreta ao DIS no rodapé. Decidido em conversa antes de construir: modelo de acesso (token sem login), quem emite (agência ou casal), âmbito do link "completo" (só logística, nunca financeiro), e granularidade (por tipo, não por fornecedor)
- [x] Moodboard — mural de inspiração partilhado entre agência e casal, área "Inspiração" no painel do evento. Grid estilo masonry, carregar imagem ou colar link (Pinterest/Instagram, com miniatura extraída no servidor via `/api/moodboard/unfurl` — CORS não permite ir buscar isso do browser), legenda opcional por item. Organizado em 9 secções por omissão (Identidade Visual, Vestido & Fato, Convites & Papelaria, Decoração & Cenografia, Flores, Cabelo & Maquilhagem, Bolo & Doces, Lembranças, Fotografia & Vídeo), semeadas sozinhas por evento; a agência/casal pode acrescentar/renomear/apagar secções. Sempre partilhado (sem distinção agency/shared — nada aqui é sensível)
- [x] Partilha do Moodboard — link tokenizado, sem conta DIS, gerado pela agência ou pelo casal (`/[locale]/moodboard/[token]`), para ver **e contribuir** (ex.: madrinhas, família) — ao contrário do Portal do Fornecedor, que é só leitura. Só acrescenta, nunca apaga nem gere secções (isso fica só para quem tem conta). Limite de 300 itens por moodboard como travão simples contra spam (sem infraestrutura de rate-limiting nesta app), upload validado por tipo/tamanho (máx. 8MB). Rotas públicas em `app/api/moodboard/public`, sempre via service_role key, nunca RLS aberta ao anon
- [x] Orçamento — leitura de conjunto no topo: orçamento **total por evento** (um número, editável, em vez do antigo "orçamentado" linha a linha), medidor de quanto já está comprometido com avisos aos 85%/100%, e gráfico de barras "Onde está o dinheiro" alternável entre fornecedor e categoria. Barras num só tom da marca (a identidade está no rótulo, não na cor), percentagem sempre visível (não só em hover, que não existe em ecrã táctil), cauda dobrada em "Outros (N)" para não esconder linhas em silêncio
- [x] Revisão de UX do Planeamento (ago/2026) — (a) todos os links partilháveis passam a viver na página Partilha, incluindo o do Moodboard, que tinha ficado num painel próprio a contrariar a decisão original; (b) gravação deixa de ser silenciosa: `useSaveStatus` + `SaveStatusBadge` em Orçamento/Tarefas/Cronograma/Inspiração, que também acabam com o padrão de ignorar o `error` do Supabase; (c) pontos de partida — checklist base de 23 tarefas com prazos relativos à data e 14 rubricas de orçamento a zeros (`lib/plannerTemplates.ts`), aplicáveis num clique em eventos vazios; (d) Orçamento e Inspiração ligados: uma linha de custo de "Flores" oferece atalho para as inspirações dessa secção, que fica destacada à chegada
- [x] Observações sobre o construído (ago/2026): (1) linhas "por pessoa" ganham dois modos alternativos — por escalão etário (adulto/criança/bebé, lê a categoria já preenchida no RSVP) ou com mínimo contratual + preço de excedente; mutuamente exclusivos por desenho, não combináveis num só cálculo. (2) `event_tasks.responsible` (agência/noivos/conjunta), com filtro na checklist e constraint na BD a impor que tarefa não-agência é sempre partilhada — a checklist base (23 tarefas) já vem com o responsável atribuído por item
- [ ] Modelos de checklist próprios da agência — guardar a checklist de um evento como modelo da marca, para aplicar a eventos novos. Identificado como pedido provável nº 1 do piloto (agências experientes têm métodos de 50-80 passos, não vão adotar a nossa checklist base de 23); ainda não construído
- [ ] Central de mensagens/atividade para o casal — resumo tipo "o seu wedding planner confirmou o pagamento do fotógrafo". Deixado para decidir depois do piloto: muda o destinatário do que construímos até agora (tudo serve a agência; isto serve o casal), e a forma mais robusta (registo próprio de eventos) toca em todos os pontos de escrita do módulo
- [ ] Onboarding — link enviado pela WP aos noivos para recolher informação estruturada logo no início da relação. Adiado para trabalhar mais tarde; por decidir quando retomar: se o link existe antes de o evento ser criado no DIS (primeiro contacto, sem conta) ou só depois (dentro do painel do casal)
